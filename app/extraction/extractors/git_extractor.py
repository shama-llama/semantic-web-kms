...
def main() -> None:
    ...
    g = load_or_create_graph(get_output_path("web_development_ontology.ttl"))
    ...
    with Progress(
        ...
        all_commit_data = extract_commit_data(
            repo_commit_map, get_input_dir(), progress, overall_task
        )
    ...
    g.serialize(
        destination=get_output_path("web_development_ontology.ttl"), format="turtle"
    )
    logger.info(
    ...
    console.print(
        f"[bold green]Ontology updated and saved to:[/bold green] [cyan]{get_output_path("web_development_ontology.ttl")}[/cyan]"
    )
