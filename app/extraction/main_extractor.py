...
def run_extractor(
    ...
    try:
        ...
        # Map extractor names to stage keys for progress tracking
        stage_mapping = {
            "File Extractor": "fileExtraction",
            "Content Extractor": "contentExtraction",
            "Code Extractor": "codeExtraction",
            "Documentation Extractor": "documentationExtraction",
            "Git Extractor": "gitExtraction",
        }

        stage_key = stage_mapping.get(extractor_name)
        tracker = get_current_tracker()

        if tracker and stage_key:
            # Update stage to processing
            tracker.update_stage(
                stage_key, "processing", 0, f"Starting {extractor_name}..."
            )
        ...
        if tracker and stage_key:
            tracker.update_stage(
                stage_key, "completed", 100, f"{extractor_name} completed successfully"
            )
        ...
        if tracker and stage_key:
            tracker.update_stage(stage_key, "error", 0, f"{extractor_name} failed: {e}")
...
def display_summary(results: List[ExtractionResult], console: Console) -> None:
    """
    Display a summary of all extraction results in a table.
    ...
    Returns:
        None
    Why:
        Provides a clear, user-friendly summary of pipeline status.
    """
...
def main(input_dir: Optional[str] = None, exit_on_completion: bool = True) -> None:
    """
    Orchestrate the extraction pipeline: run all extractors, summarize results, and exit with appropriate code.

    Args:
        input_dir: Optional input directory to analyze
        exit_on_completion: Whether to call sys.exit() when done (default True for standalone use)
    Returns:
        None
    Raises:
        SystemExit: Exits with code 0 if all extractors succeed, 1 otherwise (only if exit_on_completion=True).
    Why:
